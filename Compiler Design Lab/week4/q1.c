...
int generateHash(char *name) {
    int sum = 0;
    for(int i = 0; name[i] != '\0'; i++) sum += (int)name[i];
...
Token* search(char *name) {
    ...
    while (curr) {
        if (strcmp(curr->tok->token_name, name) == 0) return curr->tok;
        curr = curr->next;
    }
    return NULL;
}
...
void insert(Token t) {
    ...
    if (strcmp(t.type, "id") != 0 && !isDataType(t.type)) return;
    
    // Don't insert if it already exists
    if (search(t.token_name)) return;
...
void displaySymTable() {
    ...
    for(int i = 0; i < TABLE_LENGTH; i++) {
        ...
        while(curr != NULL) {
            int size = 0;
            if(strcmp(curr->tok->type, "int") == 0) size = 4;
            else if(strcmp(curr->tok->type, "double") == 0) size = 8;
            else if(strcmp(curr->tok->type, "char") == 0) size = 1;
            else size = 0; // Default size for non-data types
    ...
    while ((c = fgetc(f1)) != EOF) {
        ...
        if (c == '/') {
			...
			if (c == '*') {
			    ...
			    if (postStar == '/') { COL++; break; } // Found end
			    else ungetc(postStar, f1); // Not the end, put it back
			}
		}
                continue;
...
int main() {
    f1 = fopen("input.c", "r");
    if (!f1) { printf("Error opening file\n"); return 1; }
